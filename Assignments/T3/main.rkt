...
(define (interp expr env [inObject #f] [forwardObj #f])
  (match expr
    [(num n) (numV n)]
    [(bool b) (boolV b)]
    [(binop f l r) (make-val (f (open-val (interp l env inObject forwardObj))
                                (open-val (interp r env inObject forwardObj))))]
    [(unop f s) (make-val (f (open-val (interp s env inObject forwardObj))))]
    [(my-if c t f)
     (def (boolV cnd) (interp c env inObject))
     (if cnd
         (interp t env inObject forwardObj)
         (interp f env inObject forwardObj))]
    [(id x) (env-lookup x env)]
    [(seqn expr1 expr2) (begin
                          (interp expr1 env inObject forwardObj)
                          (interp expr2 env inObject forwardObj))]
    [(lcal defs body)
